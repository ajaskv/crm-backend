...
class ProductServiceController extends Controller
{
    ...
    public function create()
    {
        if (\Auth::user()->can('create product & service')) {
            ...
            $expense_id = ChartOfAccountType::where('name', 'Expenses')->where('created_by', '=', \Auth::user()->creatorId())->first()->id;
            $asset_id = ChartOfAccountType::where('name', 'Assets')->where('created_by', '=', \Auth::user()->creatorId())->first()->id;
            $expenseAccounts = ChartOfAccount::whereIn('type', [$expense_id, $asset_id])->where('created_by', '=', \Auth::user()->creatorId())->get()->pluck('name', 'id');

            return view('productservice.create', compact('category', 'unit', 'tax', 'customFields', 'expenseAccounts'));
        } else {
    ...
    public function store(Request $request)
    {
        ...
        if (\Auth::user()->can('create product & service')) {
            ...
            if ($request->type == "expense") {
                $productService->expense_account_id = $request->expense_account_id;
            } else {
                $productService->expense_account_id = null;
            }
    ...
    public function edit($id)
    {
        ...
        if (\Auth::user()->can('edit product & service')) {
            if ($productService->created_by == \Auth::user()->creatorId()) {
                ...
                return view('productservice.edit', compact('category', 'unit', 'tax', 'productService', 'customFields', 'expenseAccounts'));
            } else {
                return response()->json(['error' => __('Permission denied.')], 401);
            }
        } else {
