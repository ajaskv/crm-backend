...
class InvoiceController extends Controller
{
    ...
    public function create($customerId)
    {
        ...
        if (\Auth::user()->can('create invoice')) {
            ...
            $product_services = ProductService::where('created_by', \Auth::user()->creatorId())->whereIn('type', ['product', 'service'])->get()->pluck('name', 'id');
    ...
    public function store(Request $request)
    {
        ...
        if (\Auth::user()->can('create invoice')) {
            ...
            $journal->reference = $invoice->invoice_id;
            $journal->description = "Invoice";
            $journal->type_id = $invoice->id;
            ...
            if ($journal->save()) {
                ...
                $journalItem->description = "Invoice sub total";
                $journalItem->debit = $invoice->getTotal();
                ...
                $journalItem->description = "Sales without total";
                $journalItem->debit = 0;
                $journalItem->credit = $invoice->getSubTotal() - $invoice->getTotalDiscount();
                $journalItem->save();
                //TAX
                $journalItem = new JournalItem();
                $journalItem->journal = $journal->id;
    ...
    public function update(Request $request, Invoice $invoice)
    {
        if (\Auth::user()->can('edit bill')) {
            if ($invoice->created_by == \Auth::user()->creatorId()) {
                ...
                //journal entry
                $journal = JournalEntry::where(['type' => "invoice", 'type_id' => $invoice->id])->first();
                if (!$journal) {
                    $journal = new JournalEntry();
                    $journal->journal_id = $this->journalNumber();
    ...
    public function destroy(Invoice $invoice, Request $request)
    {
        ...
        if (\Auth::user()->can('delete invoice')) {
            if ($invoice->created_by == \Auth::user()->creatorId()) {
                ...
                $journalEntry = JournalEntry::where('type_id', $invoice->id)->where('type', 'invoice')->where('created_by', \Auth::user()->creatorId())->first();
                if ($journalEntry) {
    ...
    public function customerInvoiceShow($id)
    {
        ...
        $invoicePayment = InvoicePayment::where('invoice_id', $id)->where('cheque', 1)->first();
        $user = User::where('id', $invoice->created_by)->first();
        if ($invoice->created_by == $user->creatorId()) {
            $customer = $invoice->customer;
            $iteams = $invoice->items;
            if ($user->type == 'super admin') {
                return view('invoice.view', compact('invoice', 'customer', 'iteams', 'user', 'invoicePayment'));
    ...
    public function createPayment(Request $request, $invoice_id)
    {
        if (\Auth::user()->can('create payment invoice')) {
            ...
            if (isset($request->cheque)) {
                $invoicePayment->cheque = 1;
                $invoicePayment->cheque_no = $request->cheque_no;
                $invoicePayment->cheque_bank = $request->cheque_bank;
            ...
            if (!isset($request->cheque)) {
                ...
                $journal = new JournalEntry();
                $journal->journal_id = $this->journalNumber();
                $journal->date = $invoicePayment->date;
                $journal->reference = $invoicePayment->id;
                $journal->description = "Invoice Payment";
                $journal->type_id = $invoicePayment->id;
                $journal->type = "invoice-payment";
                $journal->created_by = \Auth::user()->creatorId();
                if ($journal->save()) {
                    //Account Recivable
                    $journalItem = new JournalItem();
                    $journalItem->journal = $journal->id;
                    $journalItem->account = ChartOfAccount::where(['created_by' => \Auth::user()->creatorId(), 'account_name' => "account-receivable"])->first()->id;
                    $journalItem->description = "Invoice Payment";
                    $journalItem->debit = 0;
                    $journalItem->credit = $invoicePayment->amount;
                    $journalItem->save();
                    ...
                    $journalItem->description = "Invoice Payment";
                    $journalItem->debit = $invoicePayment->amount;
                    $journalItem->credit = 0;
                    $journalItem->save();
                }
    ...
    public function chequePaid($id)
    {
        ...
        if ($invoicePayment->save()) {
            ...
            Utility::userBalance('customer', $invoice->customer_id, $invoicePayment->amount, 'debit');
    ...
    public function paymentDestroy(Request $request, $invoice_id, $payment_id)
    {
        ...
        if (\Auth::user()->can('delete payment invoice')) {
            ...
            $journalEntrys = JournalEntry::where('type_id', $payment_id)->where('type', 'invoice-payment')->where('created_by', \Auth::user()->creatorId())->get();
