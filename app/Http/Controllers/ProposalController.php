    ...
    public function create($customerId)
    {
        if (\Auth::user()->can('create proposal')) {
            ...
            if (\Auth::user()->pro_quo == 0) {
                ...
                $proposal_number = \Auth::user()->quotationNumberFormat($this->proposalNumber());
            }
    ...
    public function edit($ids)
    {
        ...
        if (\Auth::user()->can('edit proposal')) {
            ...
            if (\Auth::user()->pro_quo == 0) {
                ...
                $proposal_number = \Auth::user()->quotationNumberFormat($proposal->proposal_id);
            }
            $customers = Customer::where('created_by', \Auth::user()->creatorId())->get()->pluck('name', 'id');
            $category = ProductServiceCategory::where('created_by', \Auth::user()->creatorId())->where('type', 1)->get()->pluck('name', 'id');
            $category->prepend('Select Category', '');
    ...
    public function sent($id)
    {
        if (\Auth::user()->can('send proposal')) {
            ...
            if (\Auth::user()->pro_quo == 0) {
                ...
                $proposal->proposal = \Auth::user()->quotationNumberFormat($proposal->proposal_id);
            }
