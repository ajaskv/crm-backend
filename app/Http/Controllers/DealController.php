...
class DealController extends Controller
{
    ...
    public function show(Deal $deal)
    {
        if ($deal->is_active) {
            ...
            $customer = Customer::where('id', $deal->customer_id)->first();
            return view('deals.show', compact('deal', 'customFields', 'calenderTasks', 'permission', 'customers', 'proposal_number', 'product_services', 'category', 'customFields', 'customerId', 'customer'));
        } else {
            return redirect()->back()->with('error', __('Permission Denied.'));
        }
    }
    ...
    public function edit(Deal $deal)
    {
        if (\Auth::user()->can('edit deal')) {
            if ($deal->created_by == \Auth::user()->ownerId()) {
                ...
                $products = ProductService::where('created_by', '=', \Auth::user()->ownerId())->whereIn('type',['product','service'])->get()->pluck('name', 'id');
...
//                ClientPermission::where('deal_id', '=', $deal->id)->delete();
    ...
    public function productEdit($id)
    {
        if (\Auth::user()->can('edit deal')) {
            $deal = Deal::find($id);
            if ($deal->created_by == \Auth::user()->ownerId()) {
                $products = ProductService::where('created_by', '=', \Auth::user()->ownerId())->whereIn('type',['product','service'])->whereNOTIn('id', explode(',', $deal->products))->get()->pluck('name', 'id');
        ...
        if ($usr->can('edit deal')) {
            ...
            if ($deal->created_by == $usr->ownerId()) {
                if (!empty($request->products)) {
                    ...
                    $objProduct = ProductService::whereIN('id', $products)->whereIn('type',['product','service'])->get()->pluck('name', 'id')->toArray();
                    ActivityLog::create(
                        [
                            'user_id' => $usr->id,
                            'deal_id' => $deal->id,
