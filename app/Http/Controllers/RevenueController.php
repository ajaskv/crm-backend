...
class RevenueController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (\Auth::user()->can('create revenue')) {
            ...
            if (isset($request->cheque)) {
                $revenue->cheque = 1;
                $revenue->cheque_no = $request->cheque_no;
                $revenue->cheque_bank = $request->cheque_bank;
                $revenue->cheque_date = $request->cheque_date;
            }
            $revenue->save();
            ...
            if (!isset($request->cheque)) {
                ...
                $journal->date = $revenue->date;
                $journal->reference = $revenue->id;
                $journal->description = "Revenue";
                $journal->type_id = $revenue->id;
                ...
                if ($journal->save()) {
                    ...
                    $journalItem->description = "Revenue";
                    $journalItem->debit = 0;
                    $journalItem->credit = $revenue->amount;
                    $journalItem->save();
                    ...
                    $journalItem->description = "Revenue";
                    $journalItem->debit = $revenue->amount;
    ...
    public function chequePaid($id)
    {
        ...
        if ($revenue->save()) {
            ...
            $revenue->user_id = $revenue->customer_id;
            $revenue->user_type = 'Customer';
            $revenue->account = $revenue->account_id;

            Transaction::addTransaction($revenue);

            if (!empty($revenue->customer_id)) {
            ...
            return redirect()->route('revenue.index')->with('success', __('Cheque paid successfully created.') . ((isset($smtp_error)) ? '<br> <span class="text-danger">' . $smtp_error . '</span>' : ''));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    ...
    public function chequeNotPaid($id)
    {
        ...
        $revenue->save();
        $revenue->delete();
        return redirect()->route('revenue.index')->with('success', __('Cheque Cancelled successfully ') . ((isset($smtp_error)) ? '<br> <span class="text-danger">' . $smtp_error . '</span>' : ''));
    }
    ...
    public function update(Request $request, Revenue $revenue)
    {
        if (\Auth::user()->can('edit revenue')) {
            ...
            if (!isset($request->cheque)) {
                ...
                //journal entry
                $journal = JournalEntry::where('type_id', $revenue->id)->where('type', 'revenue')->where('created_by', \Auth::user()->creatorId())->first();
                $journal->date = $revenue->date;
                if ($journal->save()) {
                    //Account Recivable
                    $account_id = ChartOfAccount::where(['created_by' => \Auth::user()->creatorId(), 'account_name' => "account-receivable"])->first()->id;
    ...
    public function destroy(Revenue $revenue)
    {
        if (\Auth::user()->can('delete revenue')) {
            if ($revenue->created_by == \Auth::user()->creatorId()) {
                ...
                $journalEntry = JournalEntry::where('type_id', $revenue->id)->where('type', 'revenue')->where('created_by', \Auth::user()->creatorId())->first();
                if ($journalEntry) {
                    $journalEntry->delete();
