...
class SystemController extends Controller
{
        ...
        if (\Auth::user()->can('manage company settings')) {
            ...
            $request->validate(
                [
                    ...
                    'company_seal' => 'image|mimes:png|max:20480',
                ]
            );
            $post = $request->all();
            unset($post['_token']);
            ...
            if (isset($post['company_seal'])) {
                $landingSeal =  $user->id.'company_seal.png';
                @unlink('storage/uploads/seal/'. $landingSeal);
                $path = $request->file('company_seal')->storeAs('uploads/seal/', $landingSeal);
                $post['company_seal'] = $landingSeal;
            }
            $settings = Utility::settings();
            foreach ($post as $key => $data) {
                // if($key == 'company_seal'){
    ...
    public function companyIndex()
    {
        if (\Auth::user()->can('manage company settings')) {
            ...
            $mail = EmailConfig::where('user_id', Auth::user()->creatorId())->first();
            return view('settings.company', compact('mail', 'settings', 'company_payment_setting', 'timezones'));
        } else {
            return redirect()->back()->with('error', 'Permission denied.');
        }
    }
    ...
    public function saveMailSettings(Request $request)
    {
        ...
        $mail = EmailConfig::where('user_id', Auth::user()->id)->first();
        if (!isset($mail)) {
            $mail = new EmailConfig();
            $mail->user_id = Auth::user()->id;
        }
        $mail->mail_driver = $request->mail_driver;
        ...
        $mail->save();
        return redirect()->back()->with('success', __('Mail setting successfully updated.'));
    }
    public function saveMailChimpSettings(Request $request)
    {
        ...
        $mail->mailchimp_api_key = $request->mailchimp_api_key;
        $mail->mailchimp_list_id = $request->mailchimp_list_id;
        $mail->mailchimp_from_email = $request->mailchimp_from_email;
        $mail->mailchimp_from_name = $request->mailchimp_from_name;
        $mail->mailchimp_to_name = $request->mailchimp_to_name;
        $mail->save();
        return redirect()->back()->with('success', __('MailChimp setting successfully updated.'));
    }
    ...
    public function setView($type, $value)
    {
        switch ($type) {
            case 'pro_quo':
                \Auth::user()->pro_quo = $value;
                \Auth::user()->save();
                break;
            case 'bill_ipo':
                \Auth::user()->bill_ipo = $value;
                \Auth::user()->save();
                break;
            default:
                # code...
