...
@section('action-button')
    <div class="row d-flex justify-content-end">
            ...
            <div class="col-xl-2 col-lg-3 col-md-4 col-sm-12 col-12">
                <div class="all-select-box">
                    <div class="btn-box">
                        ...
                        {{ Form::select('default_pipeline_id', $pipelines, $pipeline->id, ['class' => 'form-control select2', 'id' => 'default_pipeline_id']) }}
                        {{ Form::close() }}
                    </div>
                </div>
            </div>
        ...
        @can('create deal')
            <div class="col-xl-2 col-lg-3 col-md-4 col-sm-12 col-12">
                <div class="all-button-box">
                    <a href="#" data-url="{{ route('deals.create') }}" data-ajax-popup="true" data-size="lg"
                        data-title="{{ __('Create Deal') }}" class="btn btn-xs btn-white btn-icon-only width-auto"><i
        ...
        <div class="col-xl-2 col-lg-3 col-md-4 col-sm-12 col-12">
            <div class="all-button-box">
                <a href="{{ route('deals.list') }}" class="btn btn-xs btn-white btn-icon-only width-auto"><i
                        class="fas fa-list"></i> {{ __('List View') }} </a>
            </div>
        </div>
    </div>
@endsection
                ...
                <div class="board" data-plugin="dragula" data-containers='{!! json_encode($json) !!}'>
                    ...
                    @foreach ($stages as $stage)
                        ...
                        <div class="tasks mb-2">
                            <h5 class="mt-0 mb-0 task-header">{{ $stage->name }} (<span
                                    class="count">{{ count($deals) }}</span>)</h5>
                                                    ...
                                                    @else
                                                        <div class="dropdown global-icon pr-1">
                                                            ...
                                                            <div class="dropdown-menu dropdown-menu-right">
                                                                ...
                                                                @endcan
                                                                @can('delete deal')
                                                                    <a href="#" data-title="{{ __('Delete Deal') }}"
                                                                        class="dropdown-item"
                                                                        data-confirm="{{ __('Are You Sure?') . '|' . __('This action can not be undone. Do you want to continue?') }}"
                                                                        data-confirm-yes="document.getElementById('delete-form-{{ $deal->id }}').submit();">{{ __('Delete') }}</a>
                                                ...
                                                <h6>
                                                    ...
                                                    @if ($labels)
                                                        @foreach ($labels as $label)
                                                            <span class="badge badge-{{ $label->color }} mr-1"
                                                                data-toggle="tooltip"
                                                                data-original-title="{{ $label->name }}"> </span>
                                                        @endforeach
                                                    @endif
                                                </h6>
                                                <h5 class="mt-2 mb-2">
...
@else
# @endcan" class="text-body">{{ $deal->name }} <span class="deal_icon"><i
                                                            class="fas fa-eye"></i></span></a>
                                            </h5>
                                            <p class="mb-0">
                                                <span class="text-nowrap mb-2 d-inline-block text-xs">
                                                    <b>{{ count($deal->tasks) }}/{{ count($deal->complete_tasks) }}</b>
                                                    {{ __('Tasks') }}
                                                </span>
                                            </p>
                                            <div class="float-right font-weight-bold mt-1 text-sm">
                                                {{ \Auth::user()->priceFormat($deal->price) }}</div>
                                            <p class="mb-0">
