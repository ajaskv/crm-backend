    ...
    <div class="row">
        <div class="col-12">
            <ul class="nav nav-tabs" id="myTab" role="tablist">
                <li>
                    <a class="active show" id="profile-tab" data-toggle="tab" href="#profile" role="tab"
                ...
                <div class="tab-pane fade active show" id="profile" role="tabpanel" aria-labelledby="profile-tab">
                    <div class="form-group m-0">
                        <form method="post" id="form-comment"
                            data-action="{{ route('bug.comment.store', [$bug->project_id, $bug->id]) }}">
                            <textarea class="form-control" name="comment" placeholder="{{ __('Write message') }}" id="example-textarea"
                                rows="3" required></textarea>
                            <div class="text-right mt-1">
                                <div class="btn-group mb-2 ml-2 d-none d-sm-inline-block">
                                    <button type="button"
                                        class="btn badge-blue btn-xs rounded-pill my-auto text-white">{{ __('Submit') }}</button>
                                </div>
                            </div>
                        </form>
                                ...
                                <div class="media">
                                    <div class="media-body">
                                        <div class="d-flex justify-content-between align-items-end">
                                            <div>
                                                <h5 class="mt-0">
                                                    {{ !empty($comment->user) ? $comment->user->name : '' }}</h5>
                                                <p class="mb-0 text-xs">{{ $comment->comment }}</p>
                                            </div>
                                            <a href="#" class="btn btn-outline btn-sm text-danger delete-comment"
                                                data-url="{{ route('bug.comment.destroy', $comment->id) }}">
                ...
                <div class="tab-pane fade" id="contact" role="tabpanel" aria-labelledby="contact-tab">
                    <div class="form-group m-0">
                        <form method="post" id="form-file" enctype="multipart/form-data"
                            data-url="{{ route('bug.comment.file.store', $bug->id) }}">
                            @csrf
                            ...
                            <div class="text-right mt-1">
                                <div class="btn-group mb-2 ml-2 d-none d-sm-inline-block">
                                    <button type="submit"
                                        class="btn badge-blue btn-xs rounded-pill my-auto text-white">{{ __('Upload') }}</button>
                                </div>
                            </div>
                        </form>
                                ...
                                <div class="col-4 mb-2 file-{{ $file->id }}">
                                    <div class="comment-trash" style="float: right">
                                        <a download href="{{ asset(Storage::url('bugs/' . $file->file)) }}"
                                            class="btn btn-outline btn-sm text-primary m-0 px-2">
                                            <i class="fa fa-download"></i>
                                        </a>
                                        <a href="#"
                                            class="btn btn-outline btn-sm red text-danger delete-comment-file m-0 px-2"
                                            data-id="{{ $file->id }}"
                                            data-url="{{ route('bug.comment.file.destroy', [$file->id]) }}">
