    ...
    <tbody>
        ...
        @if (isset($allProjects) && $allProjects == true)
            @foreach ($timesheetArray as $key => $timesheet)
                <tr>
                    <td colspan="9"><span class="project-name font-weight-700">{{ $timesheet['project_name'] }}</span>
                    </td>
                </tr>
                        ...
                        <tr class="timesheet-user">
                            ...
                            @foreach ($dateTimeArray['week'] as $dateSubArray)
                                <td><span class="task-time" data-type="{{ $dateSubArray['type'] }}"
                                        ...
                                        data-url="{{ $dateSubArray['url'] }}">{{ $dateSubArray['time'] != '00:00' ? $dateSubArray['time'] : '-' }}</span>
                                </td>
                            @endforeach
            ...
            @foreach ($timesheetArray as $key => $timesheet)
                <tr class="">
                    ...
                    @foreach ($timesheet['dateArray'] as $day => $datetime)
                        <td><span class="task-time" data-type="{{ $datetime['type'] }}"
                                data-task-id="{{ $timesheet['task_id'] }}" data-date="{{ $datetime['date'] }}"
                                data-ajax-timesheet-popup="true"
                                data-url="{{ $datetime['url'] }}">{{ $datetime['time'] != '00:00' ? $datetime['time'] : '-' }}</span>
                        </td>
                    @endforeach
