    ...
    <div class="details mb-2">
        <div class="form-group text-center">
            <label for="descriptions"
                class="form-control-label">{{ $parseArray['project_name'] . ' : ' . $parseArray['task_name'] }}</label>
            <small class="form-text text-muted mb-2 mt-0">{{ __('Total Time worked on this task') }} :
                {{ $parseArray['totaltaskhour'] . ' ' . __('Hours') . ' ' . $parseArray['totaltaskminute'] . ' ' . __('Minutes') }}</small>
        </div>
    </div>
