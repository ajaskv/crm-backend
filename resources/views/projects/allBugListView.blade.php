                                ...
                                @foreach ($bugs as $bug)
                                    <tr>
                                        <td>
                                            ...
                                            <span
                                                class="d-block text-sm text-muted">{{ !empty($bug->project) ? $bug->project->project_name : '' }}
                                                <span
                                                    class="badge badge-xs badge-{{ \Auth::user()->checkProject($bug->project_id) == 'Owner' ? 'success' : 'warning' }}">{{ __(\Auth::user()->checkProject($bug->project_id)) }}</span>
                                            </span>
                                        </td>
                                        <td>{{ !empty($bug->bug_status) ? $bug->bug_status->title : '' }}</td>
                                        <td>
                                            <span
                                                class="badge badge-pill badge-sm badge-{{ __(\App\Models\ProjectTask::$priority_color[$bug->priority]) }}">{{ __(\App\Models\ProjectTask::$priority[$bug->priority]) }}</span>
                                        </td>
                                                ...
                                                @if ($bug->users()->count() > 0)
                                                    ...
                                                    <a href="#" class="avatar rounded-circle avatar-sm">
                                                        <img data-original-title="{{ !empty($user[0]) ? $user[0]->name : '' }}"
                                                            @if ($user[0]->avatar) src="{{ asset('/storage/uploads/avatar/' . $user[0]->avatar) }}" @else src="{{ asset('assets/img/avatar/avatar-1.png') }}" @endif
