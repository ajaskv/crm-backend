            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    var html = '<div class="col-8 mb-2 file-' + data.id + '">' +
                        ...
                        '                                        </a>' +
                        '                                        <a href="#" class="btn btn-outline btn-sm red text-danger delete-comment-file m-0 px-2" data-id="' +
    ...
    <div class="all-button-box row d-flex justify-content-end">
        @can('create bug report')
            <div class="col-xl-2 col-lg-2 col-md-4 col-sm-6 col-6">
                <a href="#" data-url="{{ route('task.bug.create', $project->id) }}" data-ajax-popup="true"
                    data-title="{{ __('Create Bug') }}" class="btn btn-xs btn-white btn-icon-only width-auto"><i
                        class="fas fa-plus"></i> {{ __('Create') }}</a>
            </div>
        @endcan
        @can('manage bug report')
            <div class="col-xl-2 col-lg-2 col-md-4 col-sm-6 col-6">
                <a href="{{ route('task.bug', $project->id) }}" class="btn btn-xs btn-white btn-icon-only width-auto"><i
                        class="fas fa-list"></i> {{ __('Bug List') }} </a>
            </div>
        @endcan
        @can('manage project')
            <div class="col-xl-2 col-lg-2 col-md-4 col-sm-6 col-6">
        ...
        <div class="col-12">
            ...
            <div class="board" data-plugin="dragula" data-containers='{!! json_encode($json) !!}'>
                @foreach ($bugStatus as $status)
                    ...
                    <div class="tasks">
                        <h5 class="mt-0 mb-0 task-header">{{ $status->title }} (<span
                                class="count">{{ count($bugs) }}</span>)</h5>
                        <div id="lead-list-{{ $status->id }}" data-id="{{ $status->id }}"
                            class="task-list-items for-bugs mb-2">
                            @foreach ($bugs as $bug)
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-right">
                                                        ...
                                                        @can('delete bug report')
                                                            <a class="dropdown-item" href="#"
                                                                ...
                                                                data-confirm-yes="document.getElementById('delete-form-{{ $bug->id }}').submit();">{{ __('Delete') }}</a>
                                            ...
                                            <div class="my-2">
                                                <span>
                                                    <a href="#"
                                                        ...
                                                        data-ajax-popup="true" data-title="{{ __('Bug Report') }}"
                                                        class="text-body h6">{{ $bug->title }}</a>
                                                </span>
                                                @if ($bug->priority == 'low')
                                                    <span
                                                        class="font-weight-600 badge badge-xs badge-success">{{ ucfirst($bug->priority) }}</span>
                                                @elseif($bug->priority == 'medium')
                                                    <span
                                                        class="font-weight-600 badge badge-xs badge-warning">{{ ucfirst($bug->priority) }}</span>
                                                @elseif($bug->priority == 'high')
                                                    <span
                                                        class="font-weight-600 badge badge-xs badge-danger">{{ ucfirst($bug->priority) }}</span>
                                                @endif
                                            </div>
                                            <p class="mb-0">
                                                <span
                                                    class="text-nowrap mb-2 d-inline-block text-xs">{{ !empty($bug->description) ? $bug->description : '-' }}</span>
                                            </p>
                                            <div class="row">
                                                    ...
                                                    <p class="mb-0">
                                                        <a href="#" class="btn btn-sm mr-1 p-0 rounded-circle">
                                                            ...
                                                            <img alt="image" data-toggle="tooltip"
                                                                ...
                                                                @if (isset($user[0]->avatar)) src="{{ asset('/storage/uploads/avatar/' . $user[0]->avatar) }}" @else src="{{ asset('assets/img/avatar/avatar-1.png') }}" @endif
