...
// const options = data.products.map(x => x * 2);
                    let str = '';
                    data.products.map((x, i) => {
                        console.log(x);
                    str += `<option value='${x.account}'>${x.name}</option>`

                    });
                    // select.innerHTML = str;
                    $('#account').html(`<div class="input-group">
                                        <label class='form-control-label'>Account</label></br>
                                        <select class="form-control" required="required" id="" name="account">
        ...
        @can('create debit note')
            <div class="col-xl-2 col-lg-2 col-md-4 col-sm-6 col-6">
                <a href="#" data-url="{{ route('bill.custom.debit.note') }}" data-ajax-popup="true"
                    data-title="{{ __('Create New Debit Note') }}" class="btn btn-xs btn-white btn-icon-only width-auto">
                                    ...
                                    @if (!empty($bill->debitNote))
                                        @foreach ($bill->debitNote as $debitNote)
                                            <tr class="font-style">
                                                <td class="Id">
                                                    <a
                                                        href="{{ route('bill.show', \Crypt::encrypt($debitNote->bill)) }}">{{ AUth::user()->billNumberFormat($bill->bill_id) }}
                                                    </a>
                                                </td>
