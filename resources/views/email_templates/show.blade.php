                ...
                <div class="card-body">
                    <div class="row text-xs">
                        <div class="col-3 pb-3">
                            ...
                            <p class="mb-1">{{ __('Deal Name') }} : <span
                                    class="pull-right text-primary">{deal_name}</span></p>
                            <p class="mb-1">{{ __('Deal Pipeline') }} : <span
                                    class="pull-right text-primary">{deal_pipeline}</span></p>
                            <p class="mb-1">{{ __('Deal Stage') }} : <span
                                    class="pull-right text-primary">{deal_stage}</span></p>
                            <p class="mb-1">{{ __('Deal Status') }} : <span
                                    class="pull-right text-primary">{deal_status}</span></p>
                            <p class="mb-1">{{ __('Deal Price') }} : <span
                                    class="pull-right text-primary">{deal_price}</span></p>
                            <p class="mb-1">{{ __('Deal Old Stage') }} : <span
                                    class="pull-right text-primary">{deal_old_stage}</span></p>
                            <p class="mb-1">{{ __('Deal New Stage') }} : <span
                                    class="pull-right text-primary">{deal_new_stage}</span></p>
                        </div>
                        <div class="col-3 pb-3">
                            <h6 class="font-weight-bold">{{ __('Task') }}</h6>
                            <p class="mb-1">{{ __('Task Name') }} : <span
                                    class="pull-right text-primary">{task_name}</span></p>
                            <p class="mb-1">{{ __('Task Priority') }} : <span
                                    class="pull-right text-primary">{task_priority}</span></p>
                            <p class="mb-1">{{ __('Task Status') }} : <span
                                    class="pull-right text-primary">{task_status}</span></p>
                        </div>
                        @if ($emailTemplate->name == 'Assign Lead' || $emailTemplate->name == 'Move Lead')
                            <div class="col-3 pb-3">
                                ...
                                <p class="mb-1">{{ __('Lead Pipeline') }} : <span
                                        class="pull-right text-primary">{lead_pipeline}</span></p>
                                <p class="mb-1">{{ __('Lead Stage') }} : <span
                                        class="pull-right text-primary">{lead_stage}</span></p>
                                <p class="mb-1">{{ __('Lead Old Stage') }} : <span
                                        class="pull-right text-primary">{lead_old_stage}</span></p>
                                <p class="mb-1">{{ __('Lead New Stage') }} : <span
                                        class="pull-right text-primary">{lead_new_stage}</span></p>
                            </div>
                        @endif
                        @if ($emailTemplate->name == 'Assign Estimation')
                            <div class="col-3 pb-3">
                                <h6 class="font-weight-bold">{{ __('Estimation') }}</h6>
                                <p class="mb-1">{{ __('Estimation Id') }} : <span
                                        class="pull-right text-primary">{estimation_name}</span></p>
                                <p class="mb-1">{{ __('Estimation Client') }} : <span
                                        class="pull-right text-primary">{estimation_client}</span></p>
                                <p class="mb-1">{{ __('Estimation Status') }} : <span
                                        class="pull-right text-primary">{estimation_status}</span></p>
                            </div>
                        @endif
                        <div class="col-3 pb-3">
                                        ...
                                        @foreach ($languages as $lang)
                                            <li class="text-sm font-weight-bold">
                                                <a href="{{ route('manage.email.language', [$emailTemplate->id, $lang]) }}"
                                                    class="nav-link {{ $currEmailTempLang->lang == $lang ? 'active' : '' }}">{{ Str::upper($lang) }}</a>
                                            </li>
                                        @endforeach
                                    </ul>
                                </div>
