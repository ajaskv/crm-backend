        ...
        @can('create client')
            <div class="col-xl-2 col-lg-2 col-md-4 col-sm-6 col-6">
                <a href="#" class="btn btn-xs btn-white btn-icon-only width-auto" data-ajax-popup="true" data-size="lg"
                    data-title="{{ __('Create Client') }}" data-url="{{ route('clients.create') }}"><i
            ...
            <div class="col-lg-3 col-sm-6 col-md-4">
                <div class="card profile-card">
                            ...
                            @if ($client->is_active == 1)
                                ...
                                <div class="dropdown-menu dropdown-menu-right">
                                    <a href="{{ route('clients.show', $client->id) }}"
                                        class="dropdown-item text-sm">{{ __('View') }}</a>
                                    @can('edit client')
                                        <a href="#" class="dropdown-item text-sm"
                                            data-url="{{ route('clients.edit', $client->id) }}" data-ajax-popup="true"
                                            data-title="{{ __('Edit Client') }}">{{ __('Edit') }}</a>
                                    @endcan
                                    @can('delete client')
                                        <a class="dropdown-item text-sm"
                                            data-confirm="{{ __('Are You Sure?') . '|' . __('This action can not be undone. Do you want to continue?') }}"
                                            data-confirm-yes="document.getElementById('delete-form-{{ $client->id }}').submit();">{{ __('Delete') }}</a>
                                    ...
                                    <a href="#" class="dropdown-item text-sm"
                                        ...
                                        data-ajax-popup="true" data-title="{{ __('Forgot Password') }}">
                                        {{ __('Reset Password') }}
                                    </a>
                                </div>
                            @else
                    ...
                    <div class="avatar-parent-child">
                        <img @if ($client->avatar) src="{{ asset(Storage::url('uploads/avatar/' . $client->avatar)) }}" @else src="{{ asset('assets/img/avatar/avatar-1.png') }}" @endif
                            class="avatar rounded-circle avatar-xl">
                    </div>
                    <h4 class="h4 mb-0 my-2"><a
                    ...
                    <small class="my-2" data-toggle="tooltip" data-placement="bottom"
                        data-original-title="{{ __('Last Login') }}">{{ !empty($client->last_login_at) ? $client->last_login_at : '-' }}</small>
