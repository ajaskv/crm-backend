                     ...
                     @foreach ($day_group->reverse() as $key => $day_tracks)
                         ...
                         @foreach ($name_group->reverse() as $key => $name)
                             @php
                                 ...
                                 $sdates = collect($name_array)
                                     ->pluck('start_time')
                                     ->toArray();
                                 $edates = collect($name_array)
                                     ->pluck('end_time')
                                     ->toArray();
                                 $ttag = collect($name_array)
                                     ->pluck('tag_id')
                                     ->toArray();
                             ...
                             <div class="row acc-mainmenu">
                                 <div class="col-3"> <i class="fas fa-plus accodian-plus"></i>
                                     {{ $key }}</div>
                                 ...
                                 <div class="acc-sub-menu" style="display: none;">
                                     @foreach ($name as $key => $t)
                                         <div class="row acc-sub-menu-div">
                                             ...
                                             <div class="col-1">
                                                 {{ date('M-d-Y', strtotime($t->start_time)) }}
                                             </div>
                                             <div class="col-1">
                                                 {{ date('H:i:s', strtotime($t->start_time)) }}
                                             </div>
                                             <div class="col-1">{{ date('H:i:s', strtotime($t->end_time)) }}
                                             </div>
                                             ...
                                             <div class="col-1">
                                                 <img alt="Image placeholder"
                                                     ...
                                                     data-id="{{ $t['id'] }}"
                                                     id="track-images-{{ $t['id'] }}">
                                                 <i data-id="{{ $t['id'] }}"
                                                     data-is_billable="{{ $t['is_billable'] }}" data-toggle="tooltip"
                                                     data-original-title="{{ $t['is_billable'] == 1 ? __('Click to Mark Non-Billable') : __('Click to Mark Billable') }}"
                                                     class="change_billable fas fa-dollar-sign {{ $t['is_billable'] == 1 ? 'doller-billable' : 'doller-non-billable' }}"></i>
                                                 <i class="fas fa-times text-danger mx-2 pointer remove-track "
