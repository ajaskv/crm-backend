                        ...
                        <div class="card-body">
                            <div class="timeline timeline-one-side" data-timeline-content="axis"
                                ...
                                <div class="timeline-block">
                                    ...
                                    <div class="timeline-content">
                                        ...
                                        <small><i class="fas fa-clock mr-1"></i>{{ __('Created on ') }}
                                            {{ \Auth::user()->dateFormat($bill->bill_date) }}</small>
                                    </div>
                                </div>
                                <div class="timeline-block">
                                    ...
                                    <div class="timeline-content">
                                        ...
                                        @if ($bill->status == 0)
                                            <div class="Action">
                                                @can('send bill')
                                                    <a href="{{ route('bill.sent', $bill->id) }}" class="send-icon float-right"
                                                        data-toggle="tooltip" data-original-title="{{ __('Mark Sent') }}"><i
                                        ...
                                        @if ($bill->status != 0)
                                            <small><i class="fas fa-clock mr-1"></i>{{ __('Sent on') }}
                                                {{ \Auth::user()->dateFormat($bill->send_date) }}</small>
                                        @else
                                            @can('send bill')
                                            ...
                                            @can('create payment bill')
                                                <a href="#" data-url="{{ route('bill.payment', $bill->id) }}"
                                                    ...
                                                    data-original-title="{{ __('Add Payment') }}"><i
                                                        class="fas fa-money-bill-alt"></i></a> <br>
                                            @endcan
                                        @endif
                                ...
                                @if (!empty($billPayment))
                                    @if($billPayment->cheque == 1)
                                    <div class="timeline-block">
                                        <span class="timeline-step timeline-step-sm bg-info border-info text-white"><i
                                                class="far fa-money-bill-alt"></i></span>
                                        <div class="timeline-content">
                                            <div class="text-sm h6 ">{{ __('Cheque pending') }}</div>   
                                    <a href="{{ route('bill.cheque.pay', $billPayment->id) }}" class="edit-icon float-right" data-toggle="tooltip"
                                        data-original-title="{{ __('Cheque Pay') }}"
                                        data-confirm="{{ __('Are You Sure?') . '|' . __('This action can not be undone. Do you want to continue?') }}"
                                        data-confirm-yes="window.location.href = '{{ route('bill.cheque.pay', $billPayment->id) }}'"; >
                                        <i class="fas fa-thumbs-up ,"></i>
                                    </a>
                                    <a href="{{ route('bill.cheque.cancel', $billPayment->id) }}" class="delete-icon float-right" data-toggle="tooltip"
                                        data-original-title="{{ __('Cheque Cancel') }}"
                                        data-confirm="{{ __('Are You Sure?') . '|' . __('This action can not be undone. Do you want to continue?') }}"
                                        data-confirm-yes="window.location.href = '{{ route('bill.cheque.cancel', $billPayment->id) }}'"; >
                                        <i class="fas fa-times"></i>
                                    </a>
                                    <small>Cheque No:{{ $billPayment->cheque_no }} | Cheque Date:{{ $billPayment->cheque_date }} | Cheque Bank:{{ $billPayment->cheque_bank }}</small>
                                    </div>
                                </div>
                                    @endif
                                @endif
        ...
        @if ($bill->status != 0)
            <div class="row justify-content-between align-items-center mb-3">
                <div class="col-md-12 d-flex align-items-center justify-content-between justify-content-md-end">
                    ...
                    @endif
                    <div class="all-button-box mx-2">
                        <a href="{{ route('bill.resent', $bill->id) }}"
                            class="btn btn-xs btn-white btn-icon-only width-auto">
                            @if (\Auth::user()->bill_ipo == 0)
                            {{ __('Resend Bill') }}
                        ...
                        <div class="invoice-print">
                            <div class="row invoice-title mt-2">
                                <div class="col-xs-12 col-sm-12 col-nd-6 col-lg-6 col-12">
                                    ...
                                    @else
                                    <h2>{{ Auth::user()->ipoNumberFormat($bill->bill_id) }}</h2>
                                    @endif
                                            ...
                                            <tfoot>
                                                ...
                                                @if ($bill->discount_apply == 1)
                                                    <tr>
                                                        ...
                                                        <td class="text-right">
                                                            {{ \Auth::user()->priceFormat($bill->getTotalDiscount()) }}
                                                        </td>
                                                    </tr>
                                                @endif
                                                @if (!empty($taxesData))
