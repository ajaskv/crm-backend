...
@section('action-button')
    ...
    <a href="#" data-size="lg" data-url="{{ route('support.create') }}" data-toggle="tooltip" data-ajax-popup="true"
        data-title="{{ __('Create New Support') }}" class="btn btn-xs btn-white btn-icon-only width-auto">
                            ...
                            <div class="col text-center">
                                <span class="h6 mb-0">
                                    @if ($support->priority == 0)
                                        <span class="text-capitalize badge badge-primary rounded-pill badge-sm">
                                            {{ __(\App\Models\Support::$priority[$support->priority]) }}</span>
                                    @elseif($support->priority == 1)
                                        <span class="text-capitalize badge badge-info rounded-pill badge-sm">
                                            {{ __(\App\Models\Support::$priority[$support->priority]) }}</span>
                                    @elseif($support->priority == 2)
                                        <span class="text-capitalize badge badge-warning rounded-pill badge-sm">
                                            {{ __(\App\Models\Support::$priority[$support->priority]) }}</span>
                                    @elseif($support->priority == 3)
                                        <span class="text-capitalize badge badge-danger rounded-pill badge-sm">
                                            {{ __(\App\Models\Support::$priority[$support->priority]) }}</span>
                                    @endif
                                </span>
                        ...
                        <div class="row align-items-center">
                            ...
                            <div class="col text-right">
                                <a href="{{ route('support.reply', \Crypt::encrypt($support->id)) }}"
                                    data-title="{{ __('Support Reply') }}" class="action-item" data-toggle="tooltip"
