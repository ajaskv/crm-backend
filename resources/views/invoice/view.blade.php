                                ...
                                <div class="timeline-block">
                                    ...
                                    <div class="timeline-content">
                                        ...
                                        <small><i class="fas fa-clock mr-1"></i>{{ __('Created on ') }}
                                            {{ $user->dateFormat($invoice->issue_date) }}</small>
                                    </div>
                                </div>
                                <div class="timeline-block">
                                    ...
                                    <div class="timeline-content">
                                        ...
                                        @if ($invoice->status != 0)
                                            <small><i class="fas fa-clock mr-1"></i>{{ __('Sent on') }}
                                                {{ $user->dateFormat($invoice->send_date) }}</small>
                                        @else
                                            @can('send invoice')
                         ...
                         @if ($invoicePayment)
                                <div class="timeline-block">
                                    <span class="timeline-step timeline-step-sm bg-info border-info text-white"><i
                                            class="far fa-money-bill-alt"></i></span>
                                    <div class="timeline-content">
                                        <div class="text-sm h6 ">{{ __('Cheque pending') }}</div>   
                                <a href="{{ route('invoice.cheque.pay', $invoicePayment->id) }}" class="edit-icon float-right" data-toggle="tooltip"
                                    data-original-title="{{ __('Cheque Pay') }}"
                                    data-confirm="{{ __('Are You Sure?') . '|' . __('This action can not be undone. Do you want to continue?') }}"
                                    data-confirm-yes="window.location.href = '{{ route('invoice.cheque.pay', $invoicePayment->id) }}'"; >
                                    <i class="fas fa-thumbs-up ,"></i>
                                </a>
                                <a href="{{ route('invoice.cheque.cancel', $invoicePayment->id) }}" class="delete-icon float-right" data-toggle="tooltip"
                                    data-original-title="{{ __('Cheque Cancel') }}"
                                    data-confirm="{{ __('Are You Sure?') . '|' . __('This action can not be undone. Do you want to continue?') }}"
                                    data-confirm-yes="window.location.href = '{{ route('invoice.cheque.cancel', $invoicePayment->id) }}'"; >
                                    <i class="fas fa-times"></i>
                                </a>
                                <small>Cheque No:{{ $invoicePayment->cheque_no }} | Cheque Date:{{ $invoicePayment->cheque_date }} | Cheque Bank:{{ $invoicePayment->cheque_bank }}</small>
                                </div>
                            </div>
                                @endif
                            </div>
                                        ...
                                        <table class="table mb-0 table-striped">
                                            ...
                                            @foreach ($iteams as $key => $iteam)
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="text-right">
                                                        {{ $user->priceFormat($iteam->price * $iteam->quantity) }}</td>
                                                </tr>
                                            @endforeach
                                            <tfoot>
                                                <tr>
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="text-right">
                                                        {{ $user->priceFormat($invoice->getSubTotal()) }}</td>
                                                </tr>
                                                @if ($invoice->discount_apply == 1)
                                                    <tr>
                                                        <td colspan="6"></td>
                                                        ...
                                                        <td class="text-right">
                                                            {{ $user->priceFormat($invoice->getTotalDiscount()) }}</td>
                                                    </tr>
                                                @endif
                                                @if (!empty($taxesData))
                            ...
                            <div class="card bg-none card-box">
                                <section class="nav-tabs p-2">
                                    ...
                                    <div class="tab-content">
                                        @if (Utility::getValByName('enable_stripe') == 'on')
                                            <div class="tab-pane {{ (Utility::getValByName('enable_stripe') == 'on' && Utility::getValByName('enable_paypal') == 'on') || Utility::getValByName('enable_stripe') == 'on' ? 'show active' : '' }}"
                                                id="stripe_payment">
                                                <form method="post"
                                        ...
                                        @endif
                                        @if (Utility::getValByName('enable_stripe') == 'on')
                                            <div class="tab-pane fade {{ Utility::getValByName('enable_stripe') != 'on' && Utility::getValByName('enable_paypal') == 'on' ? 'active' : '' }}"
                                                id="paypal_payment">
                                                <form method="post"
