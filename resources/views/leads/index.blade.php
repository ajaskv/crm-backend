...
@section('action-button')
    <div class="row d-flex justify-content-end">
        ...
        @can('create lead')
            <div class="col-xl-2 col-lg-3 col-md-4 col-sm-12 col-12">
                <div class="all-button-box">
                    <a href="#" data-url="{{ route('leads.create') }}" data-ajax-popup="true" data-size="lg"
                        data-title="{{ __('Create Lead') }}" class="btn btn-xs btn-white btn-icon-only width-auto"><i
        ...
        <div class="col-xl-2 col-lg-3 col-md-4 col-sm-12 col-12">
            <div class="all-button-box">
                <a href="{{ route('leads.list') }}" class="btn btn-xs btn-white btn-icon-only width-auto"><i
                ...
                <div class="board" data-plugin="dragula" data-containers='{!! json_encode($json) !!}'>
                    @foreach ($lead_stages as $lead_stage)
                        ...
                        <div class="tasks mb-2">
                            <h5 class="mt-0 mb-0 task-header">{{ $lead_stage->name }} (<span
                                    class="count">{{ count($leads) }}</span>)</h5>
                            <div id="task-list-{{ $lead_stage->id }}" data-id="{{ $lead_stage->id }}"
                                class="task-list-items for-leads">
                                    ...
                                    <div class="card mb-2 mt-0" data-id="{{ $lead->id }}">
                                        <div class="card-body p-0 deal_title">
                                                    ...
                                                    @else
                                                        <div class="dropdown global-icon lead-dropdown pr-1">
                                                            ...
                                                            <div class="dropdown-menu dropdown-menu-right">
                                                                ...
                                                                @endcan
                                                                @can('delete lead')
                                                                    <a href="#" data-title="{{ __('Delete Lead') }}"
                                                                        class="dropdown-item"
                                                                        data-confirm="{{ __('Are You Sure?') . '|' . __('This action can not be undone. Do you want to continue?') }}"
                                                                        data-confirm-yes="document.getElementById('delete-form-{{ $lead->id }}').submit();">{{ __('Delete') }}</a>
                                                ...
                                                @if ($labels)
                                                    <h6>
                                                        @foreach ($labels as $label)
                                                            <span
                                                                class="badge badge-pill badge-xs badge-{{ $label->color }} mr-1"
                                                                data-toggle="tooltip"
                                            ...
                                            <a href="@can('view lead') @if ($lead->is_active){{ route('leads.show', $lead->id) }}@else# @endif @else#@endcan"
                                                class="text-body">{{ $lead->name }} <span
                                                    class="deal_icon"><i class="fas fa-eye"></i></span></a>
                                        </h5>
                                        <p class="mb-0">
