...
@section('content')
    <div class="card overflow-hidden">
        <div class="container-kanban">
            <div class="kanban-board min-750" @if (isset($permissions) && in_array('move task', $permissions)) data-plugin="dragula" @endif
                data-containers='{{ json_encode($stageClass) }}'>
                        ...
                        <div class="card-list card-list-flush">
                            ...
                            <div class="card-list-body task-list-items" id="task-list-{{ $stage->id }}"
                                ...
                                @foreach ($stage->tasks as $taskDetail)
                                    <div class="card card-progress @if (isset($permissions) && in_array('move task', $permissions)) draggable-item @endif border shadow-none"
                                        id="{{ $taskDetail->id }}"
                                        ...
                                        <div class="card-body">
                                            <div class="row align-items-center mb-2">
                                                ...
                                                <div class="col-6 text-right">
                                                    @if (str_replace('%', '', $taskDetail->taskProgress()['percentage']) > 0)
                                                        <span
                                                            class="text-sm">{{ $taskDetail->taskProgress()['percentage'] }}</span>
                                                    @endif
                                                    ...
                                                    <div class="actions d-inline-block">
                                                        @if (count($taskDetail->taskFiles) > 0)
                                                            <div class="action-item mr-2"><i
                                                                    class="fas fa-paperclip mr-2"></i>{{ count($taskDetail->taskFiles) }}
                                                            </div>
                                                        @endif
                                                        @if (count($taskDetail->comments) > 0)
                                                            <div class="action-item mr-2"><i
                                                                    class="fas fa-comment-alt mr-2"></i>{{ count($taskDetail->comments) }}
                                                            </div>
                                                        @endif
                                                        @if ($taskDetail->checklist->count() > 0)
                                                            <div class="action-item mr-2"><i
                                                                    class="fas fa-tasks mr-2"></i>{{ $taskDetail->countTaskChecklist() }}
                                                            </div>
                                                        @endif
                                                    </div>
                                                </div>
                                                                ...
                                                                @if ($key < 3)
                                                                    <a href="#" class="avatar rounded-circle avatar-sm">
                                                                        <img data-original-title="{{ !empty($user) ? $user->name : '' }}"
                                                                            @if ($user) src="{{ asset('/storage/uploads/avatar/' . $user->avatar) }}" @else src="{{ asset('assets/img/avatar/avatar-1.png') }}" @endif
    ...
    $(document).ready(function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    show_toastr('{{ __('Success') }}',
                        '{{ __('Task Deleted Successfully!') }}', 'success');
                },
            });
        });
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        var html = "<div class='list-group-item px-0'>" +
                            ...
                            data.comment + "</p>" +
                            "                            <small class='d-block'>" + now +
                            "</small>" +
                            "                        </div>" +
                            "                        <div class='col-auto'><a href='#' class='delete-comment' data-url='" +
                            data.deleteUrl +
                        ...
                        show_toastr('{{ __('Success') }}',
                            '{{ __('Comment Added Successfully!') }}', 'success');
                    },
                    error: function(data) {
        ...
        $(document).on('click', '#checklist_submit', function() {
            ...
            if (name != '') {
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        var html =
                            ...
                            '                            <a href="#" class="action-item delete-checklist" role="button" data-url="' +
                            data.deleteUrl + '">' +
                ...
                show_toastr('{{ __('Error') }}', '{{ __('Please write checklist name!') }}',
                    'error');
            }
        });
        ...
        $(document).on("click", ".delete-comment-file", function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    load_task(btn.closest('.side-modal').attr('id'));
                    show_toastr('{{ __('Success') }}',
                        '{{ __('File Deleted Successfully!') }}', 'success');
