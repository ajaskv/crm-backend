        ...
        <div class="login-inner-contain">
            ...
            <div class="login-form">
                <ul class="login-menu">
                    ...
                    <li class="gray-login"><a href="{{ route('customer.login') }}">{{ __('Customer Login') }}</a>
                    </li>
                    <li class="blue-login"><a href="{{ route('vender.login') }}">{{ __('Vendor Login') }}</a></li>
                </ul>
                <div class="page-title">
                    <h5><span>{{ __('Vendor') }}</span> {{ __('Login') }}</h5>
                </div>
                    ...
                    <select name="language" id="language" class="select-box select2"
                        ...
                        @foreach (Utility::languages() as $language)
                            <option @if ($lang == $language) selected @endif
                                value="{{ route('vender.login.lang', $language) }}">{{ Str::upper($language) }}
                            </option>
                        @endforeach
                    </select>
                </a>
